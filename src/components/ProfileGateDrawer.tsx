    ...
    <>
      ...
      <div className="gate-drawer" role="dialog" aria-modal="true" aria-label="Completa tu perfil">
        ...
        <div className="gate-content">
          {/* Header */}
          <div className="mb-5">
            <h2 className="text-[17px] font-semibold leading-snug" style={{ color: "#f7f8f8" }}>
              Completa tu perfil para conectar
            </h2>
            <p className="text-[13px] mt-1.5" style={{ color: "#8a8f98" }}>
          ...
          <div className="space-y-3 mb-6">
            {check.items.map((item) => (
              <div key={item.key} className="flex items-start gap-3">
                ...
                <span className="text-[13.5px] leading-snug" style={{ color: item.ok ? "#f7f8f8" : "#b4bcd0" }}>
                  {item.label}
                  {item.key === "biografia" && item.detail && !item.ok && (
                    <span className="ml-2 text-[11.5px]" style={{ color: "#62666d" }}>
                      {item.detail}
                    </span>
                  )}
                </span>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-lg text-[14px] font-semibold text-white transition"
            style={{ background: "linear-gradient(180deg,#5e6ad2,#4d59c4)" }}
          ...
          <button
            ...
            style={{ color: "#8a8f98", background: "none", border: "none", cursor: "pointer" }}
